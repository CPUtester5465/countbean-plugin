...
`propose_transactions`, `commit_proposal`, `run_query`, `balances`, `assess_book`,
`generate_report`, `history`, `revert`, `stage_receipt`, `propose_receipt_transaction`.

`propose_transactions` is the statement importer: give it a CSV or OFX/QFX (or the id of one
already stored on the book) and the account it belongs to, and it parses it ON THE BOOK and returns
a summary — the column mapping it detected, counts, what it flagged and each payee once — under a
`proposal_id`. `commit_proposal` then writes that proposal with the accounts you chose per payee,
validated and committed like every write. The rows never pass through the model, so a statement's
size is not limited by what a model can copy.
...
None of `propose_transactions`, `stage_receipt` or `propose_receipt_transaction` writes; `commit_proposal` and `add_transactions` do.
