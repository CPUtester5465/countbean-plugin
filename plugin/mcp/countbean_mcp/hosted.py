...
class HostedNotFoundError(HostedAccessError):
    """A 404: no such book, not yours — OR a runtime route this book lacks.

    A subclass, so everything that catches ``HostedAccessError`` still does.
    It exists for the second meaning (#637, PR #654 review): the statement
    routes (§4 rows 10-12) ship in a book-runtime image that books only get on a
    manual rollout, while this plugin publishes on every push to main. A book
    still on the older image answers ``POST /statements`` with FastAPI's own
    ``404 {"detail": "Not Found"}``, and ``propose_transactions`` catches this
    to fall back to parsing in-process as it did before, instead of failing.
    """
...
class HostedBook:
    ...
    # ---- statements by reference (#637, §4 rows 10-12) --------------------
    def stage_statement(self, content_base64: str, filename: str = "") -> dict:
        """``POST /api/statements`` — store the bytes on the book, get an id.

        The bytes go over the wire ONCE, here, from the plugin process — never
        through a model's output, which is what capped a chat import at ~65
        rows. Everything after this names the statement by its id.
        """
    ...
    def propose_statement(self, statement_id: str, account: str, options: dict) -> dict:
        """``POST /api/imports`` — parse a stored statement on the book.

        Returns the bounded summary; the rows stay on the book under the
        summary's ``proposal_id``.
        """
        body = {"statement_id": statement_id, "account": account}
        body.update({k: v for k, v in options.items() if v not in ("", None)})
        return self._request("POST", "/imports", body)

    def commit_proposal(self, proposal_id: str, overrides: str = "", open_new_accounts: bool = False) -> dict:
        """``POST /api/imports/<id>/commit`` — the write, bean-check and git on the book.

        The id is checked against the proposal-id shape HERE as well as on the
        runtime, because it is interpolated into a URL path: an id carrying a
        ``/`` or ``..`` must be refused before it becomes a different route.
        """
        if not re.fullmatch(r"prp_[0-9a-f]{12}", proposal_id or ""):
            raise LedgerError(
        ...
        return self._request(
            "POST", f"/imports/{proposal_id}/commit",
            {"overrides": overrides or "", "open_new_accounts": bool(open_new_accounts)},
        )
...
def _from_http_error(exc: urllib.error.HTTPError, what: str) -> LedgerError:
    ...
    if exc.code == 404:
        ...
        # ⚠️ A third meaning since #637: a book on an older image lacks the
        # statement routes — see `HostedNotFoundError`.
        return HostedNotFoundError(
