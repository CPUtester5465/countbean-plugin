...
def _parse_csv(text: str, account: str, options: dict) -> tuple[list[_Row], dict, list[str], list[dict]]:
    ...
    rows, sign, sign_source, shape_notes, balance_check = _apply_shape(
        shape, chronological, sign_choice, options.get("opening_balance", ""),
        skipped, decimal_sep, account=account,
        balance_column=_label_or_none(table, balance_col),
    )
    ...
    mapping = {
        ...
        # #637: whether the running balance was compared with the amounts, and
        # how many rows it contradicted. `None` means the file has no balance
        # column — "not checked" must never look like "checked and fine".
...
# Tolerance for "this amount is that balance movement". Half a cent: every
# amount this module books is quantised to at least two places, so anything
# wider would let a genuinely different number pass as the same one.
_BALANCE_TOLERANCE = Decimal("0.005")

# How much of a file has to agree before arithmetic counts as a verdict. The
# rest are rows the balance disagrees with, and they are flagged one by one.
_BALANCE_QUORUM = 0.8


def _balance_orientation(account: str, balances: list) -> tuple[int, str]:
    """Which way the statement's running balance points, relative to the book.

    🔴 THE BALANCE IS THE BANK'S NUMBER, NOT BEANCOUNT'S, and for one kind of
    account the two point opposite ways. A card or loan statement shows what
    you OWE as a positive balance that rises with every charge; in the book the
    same liability is negative and a charge makes it more so. Reading a rising
    card balance as "money in" would book every purchase as a repayment — and
    #637 is the change that makes the balance decide the sign, so getting this
    backwards would turn a flagged guess into a confident wrong answer.

    So: an ``Assets:`` (or any non-liability) account is read in its own sign.
    A ``Liabilities:`` account whose statement balances are mostly positive is
    read as *amount owed* and inverted. A liability statement that already
    shows its balance negative is in the book's sign and is read as written.
    """
    if not account.startswith("Liabilities:"):
        return 1, "the account's own sign: a rising balance is money in"
    present = [b for b in balances if b is not None and b != 0]
    positive = sum(1 for b in present if b > 0)
    if present and positive > len(present) - positive:
        return -1, (
            "amount owed: this is a Liabilities account and its statement "
            "balance is positive, so a rising balance is money OUT (more owed)"
        )
    return 1, "the account's own sign: the statement shows this liability as negative"


def _row_deltas(entries: list[dict], orientation: int, opening: Decimal | None) -> list:
    """Per entry, how far the running balance moved, in the book's sign.

    ``None`` wherever there is nothing to subtract from — the first row with no
    opening balance, and any row after one whose balance cell was empty. A gap
    RESETS the chain rather than bridging it, because a delta across two rows
    is the sum of two amounts and would read as a mismatch on a row that is
    fine.
    """
    out: list = []
    previous = opening
    for entry in entries:
        ...
        if current is None or previous is None:
            ...
            out.append(orientation * (current - previous))
        previous = current
    return out


def _tally(amounts: list, deltas: list) -> tuple[int, int, int]:
    """(agree, disagree, checked) of amounts against balance movements.

    ``checked`` counts every row that HAD a movement to compare against, not
    only the ones that matched in size either way — a row the balance moved by
    a different amount entirely is evidence too, and leaving it out of the
    denominator is how a file where nothing lines up could still reach quorum.
    """
    agree = disagree = checked = 0
    for amount, delta in zip(amounts, deltas):
        if amount is None or delta is None:
            continue
        checked += 1
        if abs(delta - amount) <= _BALANCE_TOLERANCE:
            agree += 1
        elif abs(delta + amount) <= _BALANCE_TOLERANCE:
            disagree += 1
    return agree, disagree, checked


def _verdict(agree: int, disagree: int, checked: int) -> int | None:
    """+1 keep, -1 flip, None when the balance does not settle it."""
    if not checked:
        return None
    if agree >= max(1, int(checked * _BALANCE_QUORUM)):
        return 1
    if disagree >= max(1, int(checked * _BALANCE_QUORUM)):
        return -1
    return None


def _flag_balance_mismatches(pairs: list, check: dict) -> None:
    """Flag every row whose amount the running balance does not agree with.

    ⚠️ NOT CONDITIONAL ON A VERDICT, and that is the point of #637's D4. The old
    debit/credit path had the balance column detected, reported in ``mapping``,
    and never compared — 307 of 401 rows booked the wrong way with zero flags.
    A balance column that disagrees with the amounts is the loudest evidence a
    statement can carry; it is never allowed to be silent again, whether or not
    enough of the file agreed to decide the sign.
    """
    flagged = 0
    for row, delta in pairs:
        if delta is None:
            continue
        if abs(delta - row.amount) > _BALANCE_TOLERANCE:
            flagged += 1
            row.ambiguities.append(
                f"the running balance moved by {_fmt(delta)} on this row but "
                f"the amount read is {_fmt(row.amount)}; the file contradicts "
                f"itself here, so check this row against the statement."
            )
    check["rows_flagged"] = flagged


def _apply_shape(shape, entries, sign_choice, opening_balance, skipped, decimal_sep,
                 account="", balance_column=None):
    ...
    🔴 WHEN THE FILE HAS A RUNNING BALANCE, THE BALANCE DECIDES THE SIGN (#637).
    It is the one column a bank cannot label ambiguously: it either rose or it
    fell. Headers, conventions and minus signs are all opinions about direction;
    the balance is arithmetic. So every shape that has one is checked against
    it, row by row, and a row it contradicts is flagged.

    Returns ``(rows, sign, sign_source, notes, balance_check)``; the last is
    ``None`` when the file has no balance column to check against.
    """
    notes: list[str] = []
    rows: list[_Row] = []
    ...
    opening = None
    if opening_balance:
        opening = _parse_number(str(opening_balance), decimal_sep)
        if opening is None:
            raise ImporterError(
                f"opening_balance={opening_balance!r} is not a number."
            )

    balances = [e.get("balance") for e in entries]
    has_balance = sum(1 for b in balances if b is not None) >= 2
    orientation, orientation_reason = _balance_orientation(account, balances)
    deltas = _row_deltas(entries, orientation, opening) if has_balance else [None] * len(entries)
    check = None
    if has_balance:
        check = {
            "column": balance_column,
            "orientation": orientation_reason,
            "rows_checked": 0,
            "rows_agreeing": 0,
            "rows_flagged": 0,
        }
    ...
    if shape == "debit_credit":
        # 🔴 A COLUMN'S OWN SIGN IS TYPOGRAPHY; A ROW'S SIGN AGAINST ITS COLUMN
        # IS DATA. A German export writes its whole Soll column negative
        # (`-657,62`); the old code negated it again and booked every debit as
        # a deposit — measured 94 of 401 rows right, 0 flagged (#637 D4). So a
        # column is normalised by ITS OWN usual sign: a uniformly negative debit
        # column reads as magnitudes and the column decides the direction.
        # ⚠️ But not `abs()` per cell. A debit column of positives with one
        # `-4.50` is a reversal — money back IN — and reading every cell as a
        # magnitude books it out, silently (PR #654 review: main booked the
        # refund +4.50, `abs()` booked it -4.50 with flag `*`). The row whose
        # sign disagrees with the rest of its column keeps that disagreement,
        # and is flagged `!` because a reversal is a reading, not a certainty.
        column_sign = {}
        minority = {"debit": 0, "credit": 0}
        for side in ("debit", "credit"):
            values = [e[side] for e in entries if e.get(side)]
            negative = sum(1 for v in values if v < 0)
            # A tie has no usual sign; the bank convention (a positive
            # magnitude) is the default, and the negatives are the ones flagged.
            column_sign[side] = Decimal(-1) if negative * 2 > len(values) else Decimal(1)
            minority[side] = sum(1 for v in values if (v < 0) != (column_sign[side] < 0))
        uniform_negative = [s for s in ("debit", "credit") if column_sign[s] < 0 and not minority[s]]
        if uniform_negative:
            notes.append(
                f"Every value in the {' and '.join(uniform_negative)} column(s) "
                f"carry their own minus sign. The column says which way the money "
                f"moved, so they were read as magnitudes: a debit is money out "
                f"whatever sign it is written with."
            )
        mixed = [s for s in ("debit", "credit") if minority[s]]
        if mixed:
            notes.append(
                "; ".join(
                    f"{minority[s]} value(s) in the {s} column carry the opposite "
                    f"sign to the rest of that column"
                    for s in mixed
                )
                + ". Those were read as reversals (a negative debit is money "
                "back in, a negative credit is money taken back out) and each "
                "one is flagged for review."
            )

        def _normalised(entry, side):
            value = entry[side]
            if not value:
                return None, None
            reading = value * column_sign[side]
            note = None
            if reading < 0:
                note = (
                    f"the {side} column on this row is {value}, the opposite "
                    f"sign to the rest of that column; read as a reversal "
                    f"({'money back in' if side == 'debit' else 'money taken back out'}). "
                    f"Check it against the statement."
                )
            return reading, note

        base: list = []   # (entry index, side, amount under the bank convention)
        reversal_notes: dict = {}
        for index, entry in enumerate(entries):
            debit, debit_note = _normalised(entry, "debit")
            credit, credit_note = _normalised(entry, "credit")
            flagged = [n for n in (debit_note, credit_note) if n]
            if flagged:
                reversal_notes[index] = flagged
            if debit and credit:
                base.append((index, "both", credit - debit))
            elif debit:
                base.append((index, "debit", -debit))
            elif credit:
                base.append((index, "credit", credit))
            else:
                skipped.append({"row": entry["row"],
                                "reason": "neither the debit nor the credit column has a value"})

        # Per COLUMN, not per file: each column's direction is decided by the
        # rows it fills. A file whose debits and credits both disagree has its
        # labels the other way round (some banks label from their own ledger);
        # a file where only one column disagrees has that column mis-signed.
        # Either way the balance is what the book will be reconciled against,
        # so it wins — and a row it still disagrees with is flagged below.
        multiplier = {"debit": 1, "credit": 1, "both": 1}
        if sign_choice == "inverted":
            multiplier = {"debit": -1, "credit": -1, "both": -1}
        side_source = {}
        if sign_choice == "auto" and has_balance:
            for side in ("debit", "credit"):
                amounts = [a if s == side else None for _, s, a in base]
                agree, disagree, checked = _tally(amounts, [deltas[i] for i, _, _ in base])
                decided = _verdict(agree, disagree, checked)
                if decided == -1:
                    multiplier[side] = -1
                    side_source[side] = "flipped: the running balance moves the other way on these rows"
                elif decided == 1:
                    side_source[side] = "confirmed by the running balance"
            if multiplier["debit"] == multiplier["credit"] == -1:
                multiplier["both"] = -1

        for index, side, amount in base:
            entry = entries[index]
            ambiguities = list(reversal_notes.get(index, ()))
            if side == "both":
                ambiguities.append(
                    f"both the debit ({entry['debit']}) and the credit "
                    f"({entry['credit']}) column are filled on this row; netted "
                    f"to {multiplier['both'] * amount}"
                )
            rows.append(_make_row(entry, multiplier[side] * amount, ambiguities))

        if sign_choice != "auto":
            sign, sign_source = sign_choice, "argument"
        elif side_source:
            if multiplier["debit"] == multiplier["credit"]:
                sign = "normal" if multiplier["debit"] == 1 else "inverted"
                sign_source = "verified against the running balance"
            else:
                sign = "per-column"
                sign_source = (
                    "verified against the running balance, column by column: "
                    + "; ".join(f"{k} column {v}" for k, v in sorted(side_source.items()))
                )
        else:
            sign = "normal"
            sign_source = "bank convention: the debit column is money out of the account"
            if has_balance:
                notes.append(
                    "There is a running balance, but it does not agree with the "
                    "debit/credit columns on enough rows to settle their "
                    "direction. The bank convention was used, and every row the "
                    "balance contradicts is flagged."
                )

    elif shape == "signed":
        amounts = [e["amount"] for e in entries]
        sign, sign_source, extra = _decide_sign(sign_choice, amounts, deltas)
        notes.extend(extra)
        ...
        for index, entry in enumerate(entries):
            if entry["amount"] is None:
                ...
                continue
            base.append((index, "amount", entry["amount"]))
        ...
        for index, entry in enumerate(entries):
            if entry["balance"] is None:
                ...
                continue
            if deltas[index] is None:
                first = all(e["balance"] is None for e in entries[:index])
                skipped.append({
                    "row": entry["row"],
                    "reason": (
                        "first row of a running-balance file: there is no "
                        "earlier balance to subtract, so its amount cannot be "
                        "derived. Pass opening_balance= to recover it."
                        if first else
                        "the row before it has no balance, so this row's "
                        "movement cannot be told apart from that one's"
                    ),
                })
                continue
            rows.append(_make_row(entry, flip * deltas[index], []))
        if not rows:
            raise ImporterError(
        ...
        if check is not None:
            check["rows_checked"] = check["rows_agreeing"] = len(rows)
            check["note"] = "the balance IS the amount in this shape, so every row agrees by construction"
        return rows, sign, sign_source, notes, check

    if check is not None:
        pairs = [(row, deltas[index]) for row, (index, _, _) in zip(rows, base)]
        agree, _, checked = _tally([r.amount for r, _ in pairs], [d for _, d in pairs])
        check["rows_checked"], check["rows_agreeing"] = checked, agree
        # An explicit sign= is the caller overruling the file; the counts still
        # say how far apart they are, but the rows are not flagged for it.
        if sign_choice == "auto":
            _flag_balance_mismatches(pairs, check)
    return rows, sign, sign_source, notes, check


def _decide_sign(choice, amounts, deltas):
    ...
    * **undetermined** — every amount points the same way and nothing checks it.
    ...
    ``deltas`` are the balance movements already in the book's sign
    (``_row_deltas``), so a card statement's owed-positive balance is not read
    ...
    agree, disagree, checked = _tally(amounts, deltas)
    decided = _verdict(agree, disagree, checked)
    if decided == 1:
    ...
    if checked:
        notes.append(
            ...
            f"negative on {disagree}. It could not settle the sign; every row "
            f"it contradicts is flagged."
        )
...
# ------------------------------------------------------------ loud failure --
# A parse that keeps fewer than this share of the lines it looked at is refused.
# Half, because the legitimate losses are small and bounded — the first row of a
# balance-only file, a footer, a pending line with no amount — and a parse that
# loses MOST of a statement has misread it, whatever it did produce.
_MIN_PARSED_FRACTION = 0.5
# ...and only once there are enough lines for a fraction to mean anything: a
# two-line balance file legitimately yields one row.
_MIN_LINES_FOR_FRACTION = 4


def _refuse_an_implausible_parse(rows: list, skipped: list[dict], fmt: str) -> None:
    """Fail LOUDLY when a statement parses to nothing, or to almost nothing.

    🔴 A SILENT ZERO IS THE WORST THING AN IMPORTER CAN RETURN (#637). It reads
    as "this statement had no new transactions", the model says so, the customer
    believes it, and a month of spending never reaches the book. The research
    lab measured exactly that on a PDF parser (0 of 3 rows, 0 flagged); this
    module could do it too: a debit/credit file whose amount columns were all
    blank produced `proposed: []` and no error at all.

    Raised as an ``ImporterError`` — the book is untouched, and the message
    names what was skipped and why, which is what the caller needs to pick an
    override.
    """
    seen = len(rows) + len(skipped)
    reasons = Counter(s.get("reason", "") for s in skipped).most_common(3)
    why = "; ".join(f"{n} × {r}" for r, n in reasons) or "no lines were recognised as rows"
    if not rows:
        raise ImporterError(
            f"This {fmt.upper()} parsed to ZERO transactions out of {seen} "
            f"line(s), so nothing was proposed. Skipped: {why}. Check which "
            f"columns hold the date and amount and pass columns= to say so."
        )
    if seen >= _MIN_LINES_FOR_FRACTION and len(rows) < seen * _MIN_PARSED_FRACTION:
        raise ImporterError(
            f"Only {len(rows)} of {seen} lines in this {fmt.upper()} parsed as "
            f"transactions, which means the file was misread rather than that "
            f"it is mostly empty. Nothing was proposed. Skipped: {why}. Pass "
            f"columns=, date_format= or amount_shape= to fix the reading."
        )
...
def propose(
    ...
    *,
    ...
    _refuse_an_implausible_parse(rows, skipped, fmt)
